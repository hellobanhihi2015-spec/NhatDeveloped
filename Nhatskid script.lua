...
local oldGui = PlayerGui:FindFirstChild("NhatDeveloped")
...
local function openGUI()
    ...
    textTween.Completed:Connect(function()
        nhLabel.Text = "HaminhnhatA1"
        local nameTween = TweenService:Create(nhLabel, TweenInfo.new(0.3), {
            TextTransparency = 0
        })
        local nhatdz = TweenService:Create(hubLabel, TweenInfo.new(0.3), {
            TextTransparency = 0
        })
        nameTween:Play()
        hubTween:Play()
...
if settings.flingEnabled then
    ...
    if not ReplicatedStorage:FindFirstChild("nhatdzmakescript") then
        local marker = Instance.new("Decal")
        marker.Name = "nhatmakescript"
        marker.Parent = ReplicatedStorage
    end
